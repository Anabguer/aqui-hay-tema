(function () {
    ...
    "8": {
      ...
      people: {
        ...
        cine: [
          ...
          p("c2", "Aina", "Cine", IMG.p009),
          p("c3", "Eva", "Cine", IMG.p010),
          p("a1", "Dani", "Arcade", IMG.p016),
          p("a2", "Marta", "Arcade", IMG.p018),
          p("a3", "Paco", "Arcade", IMG.p109),
          p("a4", "Luz", "Arcade", IMG.p117)
        ]
      }
    },
    "12": {
      ...
      people: {
        ...
        mala: [
          p("ev", "Eva", "Karaoke", IMG.p031, { relevante: true }),
          p("b1m", "Toni", "Bar", IMG.p009),
          p("b2", "Lola", "Bar", IMG.p010),
          p("b3", "Gabi", "Bar", IMG.p016),
          p("b4", "Nuria", "Bar", IMG.p018),
          p("b5", "Raúl", "Bar", IMG.luis),
          p("d1", "Sofi", "Discoteca", IMG.p109),
          p("d2", "Kim", "Discoteca", IMG.p117),
          p("d3", "Alex", "Discoteca", IMG.p121),
          p("d4", "Vero", "Discoteca", IMG.nora),
          p("d5", "Jon", "Discoteca", IMG.p001),
          p("k2", "Mila", "Karaoke", IMG.rocio)
        ],
        parque: [
          p("lu", "Luis", "Parque", IMG.luis),
          p("q1", "Nerea", "Picnic", IMG.p010),
          p("q2", "Omar", "Mirador", IMG.p016)
        ]
      }
    }
  };
  ...
  function setPuebloAll(p) {
    roots.forEach((r) => {
      ...
      r.removeAttribute("data-crece");
      r.querySelectorAll(".dock button").forEach((b) => {
        b.classList.toggle("is-on", !b.getAttribute("data-open"));
      });
    });
    ...
    document.querySelectorAll("[data-capa-btn]").forEach((b) => {
      b.classList.toggle("is-on", b.getAttribute("data-capa-btn") === "ninguna");
    });
  }
  ...
  function setCapaAll(name) {
    roots.forEach((r) => {
      ...
      r.removeAttribute("data-consulta");
      if (name === "diario") r.setAttribute("data-diario", r.getAttribute("data-diario") || "hoy");
      r.querySelectorAll(".dock button").forEach((b) => {
  ...
  document.body.addEventListener("click", (ev) => {
    ...
    if (demo) {
      ...
      document.querySelectorAll("[data-aforo]").forEach((b) => b.classList.remove("is-on"));
      roots.forEach((r) => {
        r.removeAttribute("data-aforo");
        r.removeAttribute("data-crece");
        r.removeAttribute("data-importante");
        renderHabs(r);
      });
      ...
      if (d === "crece") {
        setPuebloAll("pleno");
        roots.forEach((r) => r.setAttribute("data-crece", "1"));
      }
      if (d === "importante") {
        setPuebloAll("pleno");
        setCapaAll("");
        roots.forEach((r) => r.setAttribute("data-importante", "1"));
      }
      return;
    }
    ...
    if (capaBtn) {
      ...
      if (v === "organizar") { roots.forEach((r) => r.setAttribute("data-paso", "tipo")); }
      if (v === "diario") { roots.forEach((r) => r.setAttribute("data-diario", "hoy")); }
      if (v === "buzon") { roots.forEach((r) => r.setAttribute("data-importante", "1")); }
      return;
    }
    const tab = ev.target.closest("[data-diario-tab]");
    if (tab) {
      roots.forEach((r) => r.setAttribute("data-diario", tab.getAttribute("data-diario-tab")));
      document.querySelectorAll("[data-diario-tab]").forEach((b) => {
        b.classList.toggle("is-on", b.getAttribute("data-diario-tab") === tab.getAttribute("data-diario-tab"));
      });
      return;
    }
    const pasoBtn = ev.target.closest("[data-paso]");
    if (pasoBtn && pasoBtn.closest(".play-root")) {
      roots.forEach((r) => r.setAttribute("data-paso", pasoBtn.getAttribute("data-paso")));
      return;
    }
    ...
    if (chip && !chip.disabled && chip.closest(".play-root")) {
      ...
      if (chip.hasAttribute("data-tipo")) {
        const cupo = chip.getAttribute("data-cupo") || "2";
        roots.forEach((r) => {
          r.setAttribute("data-cupo", cupo);
          r.querySelectorAll("[data-tipo]").forEach((c) => c.classList.toggle("is-on", c.getAttribute("data-tipo") === chip.getAttribute("data-tipo")));
        });
        return;
      }
    ...
    if (open) {
      ...
      setCapaAll(name);
      if (name === "buzon") roots.forEach((r) => r.setAttribute("data-importante", "1"));
      return;
    }
  ...
  if (hash.indexOf("aforo") >= 0 && hash.indexOf("aforo3") < 0 && hash.indexOf("aforo8") < 0 && hash.indexOf("aforo12") < 0) setAforo("12");
  if (hash.indexOf("crece") >= 0) {
    setPuebloAll("pleno");
    roots.forEach((r) => r.setAttribute("data-crece", "1"));
  }
  if (hash.indexOf("importante") >= 0) {
    setPuebloAll("pleno");
    setCapaAll("");
    roots.forEach((r) => r.setAttribute("data-importante", "1"));
  }
  if (hash.indexOf("vecinos") >= 0) { setPuebloAll("pleno"); setCapaAll("vecinos"); }
  if (hash.indexOf("buzon") >= 0 && hash.indexOf("importante") < 0) {
    setPuebloAll("pleno");
    setCapaAll("buzon");
    roots.forEach((r) => r.setAttribute("data-importante", "1"));
  }
  if (hash.indexOf("diario") >= 0 || hash.indexOf("cotilleo") >= 0) {
    setPuebloAll("pleno");
    setCapaAll("diario");
    roots.forEach((r) => r.setAttribute("data-diario", "hoy"));
  }
  if (hash.indexOf("organizar") >= 0) {
    setPuebloAll("pleno");
    setCapaAll("organizar");
    roots.forEach((r) => r.setAttribute("data-paso", "tipo"));
  }
  if (hash.indexOf("ficha") >= 0) { setPuebloAll("pleno"); setCapaAll("ficha-conocida"); }
})();
