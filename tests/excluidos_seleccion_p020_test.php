...
for ($i = 0; $i < 50; $i++) {
    $p = $svc->nuevaPartida('juego_v1', 'excl-p020-' . $i);
...
// Save antiguo con per_p020: carga y retrato siguen funcionando
$legacy = $svc->nuevaPartida('test_fixtures_v0', 'excl-p020-legacy');
...
ok(str_contains((string) $tok['url'], 'P020_'), 'retrato usa pack P020');

echo $failures === 0 ? "OK excluidos_seleccion_p020_test\n" : "FAIL excluidos_seleccion_p020_test ({$failures})\n";
exit($failures > 0 ? 1 : 0);
