...
function Invoke-Git {
    ...
    $code = $LASTEXITCODE
    return @{ Output = ($output | Out-String).TrimEnd(); Code = $code }
}
...
Write-Log '=== COMMIT + PUSH (Aquí Hay Tema) ===' -ToHost
...
$upstream = (Invoke-Git @('rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}')).Output
$remote = 'origin'
if ($upstream -match '^([^/]+)/') { $remote = $Matches[1] }
...
if ($diffStat.Output) { Write-Host $diffStat.Output; Write-Log $diffStat.Output }
Write-Log '--- Diff en stage (stat) ---' -ToHost
if ($diffCached.Output) { Write-Host $diffCached.Output; Write-Log $diffCached.Output }
...
if ([string]::IsNullOrWhiteSpace($dryAdd.Output)) {
    Write-Log 'Nada nuevo que añadir tras aplicar .gitignore.' -ToHost
} else {
